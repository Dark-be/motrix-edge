...
> 独立子项目：自带 `pyproject.toml` / `uv.lock` / `.venv`；运行前需让 `motrix-edge/src`
> 可 import（同仓部署把 `src` 加入 `PYTHONPATH`，或 editable 安装 motrix-edge）。详细见
...
robot-pipeline/      # 机械臂底层承载（独立 uv 子项目）：config / env / robot /
                     #   server / collector / utils
config/edge.yml      # 边缘节点配置（discover / policy…）
wiki/                # 设计与计划文档
tests/               # 测试

...
> **状态**：**已落地并在 master**（`BaseEnv._observe_loop` / `step_observe` 与控制线程分离，
> `/v1/health` 带 `measured_hz`）；仅剩「实机验证」1 项未勾（见 TODO），故暂不删除本计划。
...
| 验证     | 离线冒烟（相机每拍阻塞 2s）：控制 29.97 Hz / 观测 0.50 Hz |

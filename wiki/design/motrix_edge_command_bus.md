...
| `robot teleop`            | `enabled`, `mode` | 机器人        | 遥操作 / **人工接管**（enabled=true/false；mode=absolute 缺省 / delta 增量接管）   | none |
...
| `infer config set`        | `json`            | 任务级/配置级 | 按当前策略 schema 白名单设置配置项（JSON 对象，可部分）                            | none |
| `infer model`             | —                 | 任务级/配置级 | 查询 lerobot 类策略的模型路径（`pretrained_name_or_path`）                         | none |
| `infer model set`         | `path`            | 任务级/配置级 | 设置模型路径（`pretrained_name_or_path` 项快捷命令）                               | none |

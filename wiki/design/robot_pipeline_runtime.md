...
`POST /v1/capture/start` 之后，若此刻**遥操作开着**（示教 `robot teach` 或接管 `robot takeover`，
不分模式），env 会先武装一道闸门：主臂读数相对 **capture 开始那一拍**未超过阈值
（`collector.skip_until_motion.joint_eps` / `gripper_eps`）→ **本拍不记录、也不开 episode**
（文件在首帧真正要记录时才创建）；出现一次有效移动后开始记录，此后微小位移照常记录。

-   基准 = `BaseRobot.teleop_master_sample()`（控制线程每拍刷新的「最近一次成功主臂读数」，与
    下发 target 同源），判定 = `max|Δ关节|` 与 `max|Δ夹爪|` 任一超阈值；
-   主臂此刻读不到（未开启遥操作 / 主臂还没读到）→ **不武装**：宁可按原行为记录，也不静默丢帧；
-   等主臂期间遥操作被关掉（人员交回模型）→ 闸门立即解锁并开始记录，避免整轮空等；
-   整轮都没出现有效移动 → **不产出文件**（与「同一拍 drain 到 start + end」一致），并记 WARNING；
-   跳过只作用于**本轮帧头**：下一轮 `capture start` 重新武装（各自跳过自己的静止开头）；
-   可观测：日志 + `GET /v1/capture/status` 的 `head_skip`（`{"skipped": n}`；`null` = 未在跳过）。

...
| 段                  | 含义                                     | 处置                                     |
| ------------------- | ---------------------------------------- | ---------------------------------------- |
| `prefix_actions`    | 对应**过去时刻**，已被执行 → 已失效      | 丢弃（仅统计上报；同步预取下通常为 0）   |
| `execution_actions` | **实际执行段**（本次决策真正下发的部分） | 依次下发（`execution_horizon` 步）       |
| `suffix_actions`    | **后缀**，为下一块做过渡                 | 留在队列，与下一块重叠步加权融合（平滑） |

-   块以 `start_index`（首步绝对步号）对齐：块内 `index < 当前步号` 的步即 `prefix`（已失效）。
-   `execution_horizon`（E）= 实际执行段步数；`suffix_len`（S）= 过渡后缀步数（= 与下一块的重叠窗口）；
    块长 `H = prefix + E + S`（不足时按实际块长截断，`E` 至少 1）。
-   同步预取（`remaining ≤ S` 时提前拉下一块）下 `prefix` 通常为 0；当块到达晚于当前步（异步 / 延迟）时，
    块前部落在过去步号上即成为 `prefix`，被丢弃——这是 RTC 与「异步推理」兼容的关键点。

## 时序平滑（RTC 统一承载）

**问题**：块边界处若直接从旧块末步跳到新块首步，动作会跳变（机械冲击）。

**机制**：保持**重叠**——在旧块还剩余 `S` 步时提前请求下一块，新块与旧块在 `[cur, cur+S)` 重叠；
重叠步按 `aggregate_fn` 融合（默认 `weighted_average`：`0.3*旧 + 0.7*新`，对齐 lerobot
`AGGREGATE_FUNCTIONS`），块边界由此平滑衔接。

-   聚合在**入队时**完成（绝对步号 → 动作的字典），消费时只取融合后的值。
-   `suffix_len = 0` → 退化为「块耗尽才推理」（无重叠、无平滑）。
-   聚合函数：`weighted_average`（默认）/ `latest_only` / `average` / `conservative`。

## RTCManager

```python
rtc = build_rtc(policy, config)          # 会话进入时构造（持策略引用）
rtc.infer(observation)                   # 本步动作：必要时拉块 → 切分 → 入队聚合 → 返回当前步
rtc.reset()                              # 清队列 / 步号归零（策略连接不变）
rtc.configure(**params)                  # 运行期改参数（校验后生效）
rtc.status() -> dict                     # 上报：enabled / params / index / remaining / last_chunk
```

-   **步号**：`_index` 单调递增（reset 归零）；策略按 `index` 组织观测（act 用它做 `TimedObservation.timestep`）。
-   **预取**：`remaining <= suffix_len` 时同步拉下一块（缓存不断流；单次等待 ≤ 一次推理延迟）。
-   **异常 / 空块**：`policy.infer_chunk` 抛错或返回 `None` → `infer` 返回 `None`（会话跳过本步，**不升级为任务错误**）。
-   **`enabled: false`**：无 RTC 退化模式——每步请求一次、只取块首步（不做块缓存 / 重叠）。
...
policy:
    rtc:
        ...
        inference_delay: 0 # 前缀步数 D（信息性：预期块前部已失效的步数；0 = 同步）
        aggregate_fn: weighted_average # 重叠聚合：weighted_average/latest_only/average/conservative
```

约束：`E >= 1`、`S >= 0`、`D >= 0`、`aggregate_fn` 已注册；`E + S > H` 时按实际块长截断。
...
| 方法 | 路径             | 租约 | 说明                                                                        |
| ---- | ---------------- | ---- | --------------------------------------------------------------------------- |
| GET  | `/v1/infers`     | 无   | status 增加 `rtc` 字段（enabled / params / index / remaining / last_chunk） |

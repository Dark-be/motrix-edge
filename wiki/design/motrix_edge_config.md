...
包内 **package data** 只读兜底（`edge.yml` / `capture.yml`）；日志 / 可写状态目录遵循 XDG
（`XDG_STATE_HOME`，缺省回退 CWD）。CLI 入口统一在 `__main__.py`（console script
`motrix-edge` 与 `python -m motrix_edge` 共用同一 `main()`）。
...
| `policy`     | 推理节点默认 host / port；策略类型、图像参数和 action_horizon 由客户端默认值或服务端 metadata 决定；`policy.rtc` 为实时动作块参数（块长 / 执行段 / 后缀 / 聚合） | policy / rtc          |

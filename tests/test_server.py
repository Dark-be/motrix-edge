...
def test_commands_infer_ip_and_port_get_set():
    ...
    assert r.status_code == 200
    assert r.json()["status"] == "ok"  # 查询命令也走 submit，回执即数据
...
def test_commands_infer_port_set_rejects_invalid():
    """capability=infer_port_set：非法端口 → 回执 rejected（HTTP 200 透传业务拒绝，配置不变）。"""
    client = make_endpoint_client()
    lease = install_lease(client)
    r = client.post(
        "/v1/commands",
    ...
    assert r.status_code == 200  # 命令已消费，业务拒绝经回执透传
    body = r.json()
    assert body["status"] == "rejected"  # 回执状态即命令结果（不再是笼统的 error/accepted）

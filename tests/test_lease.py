...
def test_install_trusts_console_expiry():
    """过期时间由 Console 决定：Edge 信任镜像 expires_at，不按本地时钟重算。"""
    mgr, _ = make_manager(ttl=10)
    # Console 传「过去」的 expires_at → 原样保留（不重算）；过期镜像不可控制
    lease = make_lease(ttl=20, expires_at=_BASE - timedelta(seconds=999))
    mgr.install(lease)
    assert lease.expires_at == _BASE - timedelta(seconds=999)
...
def test_renew_extends_expiry_with_version():
    """续约：更高 lease_version 原地延长 expires_at（Console 决定），renewed_at 更新。"""
    mgr, clock = make_manager()
    ...
    assert renewed.expires_at == clock.t + timedelta(seconds=10)  # Console 决定的新过期时刻
...
def test_revoke_current_revokes_active_lease():
    """revoke_current：撤销当前租约（无需 id）；返回 ``(租约, 本次是否变更)``。"""
    mgr, _ = make_manager()
    assert mgr.revoke_current() == (None, False)  # 无租约
    mgr.install(make_lease(ttl=10))
    lease, changed = mgr.revoke_current()
    assert changed is True
    assert lease.lease_id == "ls_console_issued"
    assert lease.state == LeaseState.REVOKED
    assert mgr.revoke_current() == (lease, False)  # 幂等：已撤销 → 不谎报变更

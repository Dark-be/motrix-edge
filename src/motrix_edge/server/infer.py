...
class InferService:
    ...
    def _capture_status(self) -> dict | None:
        ...
        ``meta`` 是 ``capture sync`` 同步到进程的**元信息全集**（采集员 / 任务名等是其中的
        键，分类可拓展）——与 ``/v1/captures`` 的 ``capture_status`` 同构，不另设
        ``operator`` / ``task_name`` 同义顶层字段。推理会话录制 rollout 时 node 同样周期
        刷新 ``capture_status``；此处只读缓存，不因前端轮询实时请求 SDK 进程。
        """
        node = self._node
        ...
        return {
            ...
            "meta": dict(getattr(capture_status, "meta", {}) or {}),
        }

...
SendObservations / GetActions 的 **wire 语义由上层策略客户端（policy/lerobot_act）组合**，
...
class AsyncInferenceGrpcTransport(BaseTransport):
    ...
    def _target(self) -> str:
        """按 host / port **现算** gRPC target（端点变更后无需重建缓存）。"""
        return f"{self.config.get('host') or '127.0.0.1'}:{self.config.get('port')}"
    ...
    def connect(self):
        ...
        self._channel = grpc.insecure_channel(self._target(), options=grpc_channel_options(initial_backoff="0.1s"))
        try:
            grpc.channel_ready_future(self._channel).result(timeout=self.config.get("connect_timeout", 5.0))
        except Exception:
            self.close()
            raise

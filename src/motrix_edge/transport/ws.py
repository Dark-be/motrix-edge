...
class WsTransport(BaseTransport):
    """msgpack-over-websocket 传输（原 MsgpackTransport 迁移）。"""
    ...
    def connect(self):
        ...
        try:
            ...
            self.server_metadata = msgpack_numpy.unpackb(self._ws.recv(timeout=self._connect_timeout))
        except Exception:
...
# 兼容旧名（原 motrix_edge.policy.transport.MsgpackTransport）
MsgpackTransport = WsTransport

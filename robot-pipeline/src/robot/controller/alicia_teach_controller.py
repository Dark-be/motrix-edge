...
class AliciaTeachController(ArmController):
    ...
    def __init__(self, name="alicia_teach_controller"):
        ...
        # 共享内存随 connect() 获取、disconnect() 释放（不在构造时创建）：
        # robot 构造 / connect 之间任何一步抛异常时，不会在 /dev/shm 留下无人清理的残段。
        self.shm = None
    ...
    def connect(self, port: str, collect_frequency_hz: float = 30.0):
        """连接示教臂：创建状态共享内存 + 拉起采集子进程；失败则回滚（不留残段）。"""
        self.port = port
        self.collect_frequency_hz = collect_frequency_hz

        if self.shm is None:  # 重复 connect 复用已有段，不叠加创建
        ...
        self.stop_event.clear()  # 断开后重连 / 失败后重试
        ...
        try:
            ...
            self.disconnect()  # 回滚：unlink 共享内存 + 复位子进程句柄，然后向上抛
            raise
    ...
    def _get_state(self):
        if self.shared_array is None:  # 未 connect：不返回全零（会被误读为「手臂在零位」）
    ...
    def disconnect(self):
        ...
        if self.collect_process is not None and self.collect_process.is_alive():

...
step() 限速推进、观测线程每拍 build_observation() 取帧拼装；与 adapter 协定的 obs/action
形态由类常量（QPOS / IMAGE_NAMES / IMAGES）固定声明，不依赖 motrix_edge.profile。
...
ROBOT_REGISTRY = {
    ...
    "dual_alicia_piper_robot": (
        "robot.dual_alicia_piper_robot",
        "DualAliciaPiperRobot",
    ),  # 双臂：Alicia 主手 + Piper 从手

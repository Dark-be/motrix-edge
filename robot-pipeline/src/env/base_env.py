...
class BaseEnv:
    ...
    def __init__(self, robot, capture_config: dict | None = None):
        ...
        self.last_error = None  # 命令执行最近一次异常（health 上报；sticky，需重启清除）
        self.loop_error = None  # 主循环持续失败（连续 step 异常超阈值；恢复后自动清除）
    ...
    def robot_safe_stop(self):
        """安全停止（软停：停发指令并保持位姿、不断电；幂等、失败安全）：入队，由主循环执行。"""
    ...
    def health(self) -> dict:
        ...
        ``loop_error`` 覆盖「主循环在转、但硬件已持续读不出来」的降级态（连续 step
    ...
    def start(self):
        ...
        self.robot.connect()
        self._running = True
        self._thread = threading.Thread(target=self._loop, daemon=True, name=f"{self.robot.name}-env")
        self._thread.start()
    ...
    def _loop(self):
        """主循环：以 HZ 频率驱动 step()；单帧异常不退出，连续失败超阈值则降级上报。"""
    ...
    def _update_capture(self):
        ...
        调用 start() 打开文件（UUID 命名），之后每帧 collect 直接落盘；缓冲式 collector

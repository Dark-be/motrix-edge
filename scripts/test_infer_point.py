...
- 每个请求接收 msgpack 官方 flat 观测 ``{"state": ndarray, "images": {...}}``，返回一段
  **有界随机游走**的 action chunk ``{"actions": ndarray}``（``[horizon, dim]``，块间连续），
  供 edge 验证「原始块 → RTC 切分 / 预取」链路（客户端**不做缓存**：块缓存 / 三元切分 /
  时序平滑 / 预取时机统一由 ``motrix_edge.rtc`` 负责）。
...
# 保证与 policy 客户端的收发逻辑统一（见 motrix_edge/policy/openpi/contract.py、
# transport/msgpack_numpy.py）。
from motrix_edge.policy.openpi.contract import OPENPI_KEY_ACTIONS, OPENPI_KEY_STATE
...
def handle_connection(conn, core: SimInferCore) -> None:
    ...
    遵循传输契约（``WsTransport``）：服务端以**文本**回包表示错误；客户端观测
    ...
    while True:
        ...
        conn.send(packb({OPENPI_KEY_ACTIONS: action}))
